...
function computeOverlayLines(id, candles) {
    ...
    switch (id) {
        ...
        case 'bb': {
            ...
            return [toLine(upper, '#818CF8', 1), toLine(middle, '#6366F1', 1), toLine(lower, '#818CF8', 1)];
        }
        case 'supertrend': {
            ...
            {Object.entries(CHART_PERIODS).map(([key, cfg]) => (
                <button key={key} onClick={() => onPeriodChange(key)}
                    className={cn('px-2 py-1 text-[11px] font-semibold font-price rounded transition-colors',
                        period === key ? 'bg-primary-500/20 text-primary-400' : 'text-gray-500 hover:text-gray-300 hover:bg-surface-800/60')}>
                    {cfg.label}
                </button>
            ))}
        </div>
            ...
            <div className="py-1">
                {tools.map(t => (
                    <button key={t.id} onClick={() => { onSelect(activeTool === t.id ? null : t.id); onClose(); }}
                        ...
                        <span className={cn('text-xs', activeTool === t.id ? 'text-primary-400 font-medium' : 'text-gray-400')}>{t.label}</span>
                    </button>
                ))}
            </div>
        </div>
...
class CanvasChartRenderer {
    ...
    _draw() {
        ...
        for (const hl of this.hLines) {
            ...
            if (y < this._chartTop || y > this._chartBottom) continue;
            ctx.strokeStyle = hl.color || '#6366F1';
            ctx.lineWidth = 1; ctx.setLineDash([6, 4]);
            ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(this._chartRight, y); ctx.stroke();
            ctx.setLineDash([]);
            ctx.fillStyle = hl.color || '#6366F1';
...
const ZebuLiveChart = memo(function ZebuLiveChart({
    ...
    function getZeroLossBadge(zl) {
        ...
        if (dir === 'BULLISH') { color = 'bg-emerald-500/15 border-emerald-500/25 text-emerald-400'; icon = <HiTrendingUp className="w-3.5 h-3.5" />; label = 'BULLISH'; }
        else if (dir === 'BEARISH') { color = 'bg-red-500/15 border-red-500/25 text-red-400'; icon = <HiTrendingDown className="w-3.5 h-3.5" />; label = 'BEARISH'; }
            ...
            <div className="flex items-center gap-2 px-2.5 py-1.5 border-b border-edge/5 bg-surface-900/30 flex-shrink-0" ref={menuRef}>
                ...
                <div className="relative flex-shrink-0">
                    <button onClick={() => { setShowIndicatorMenu(v => !v); setShowToolsMenu(false); }}
                        className={cn('flex items-center gap-1.5 px-2 py-1 rounded text-[11px] font-semibold transition-colors',
                            activeIndicators.size > 0 ? 'bg-primary-500/15 text-primary-400 border border-primary-500/20' : 'text-gray-500 hover:text-gray-300 hover:bg-surface-800/60')}>
                ...
                <div className="relative flex-shrink-0">
                    <button onClick={() => { setShowToolsMenu(v => !v); setShowIndicatorMenu(false); }}
                        className={cn('flex items-center gap-1.5 px-2 py-1 rounded text-[11px] font-semibold transition-colors',
                            activeTool ? 'bg-primary-500/15 text-primary-400 border border-primary-500/20' : 'text-gray-500 hover:text-gray-300 hover:bg-surface-800/60')}>
                        <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                ...
                {hLines.length > 0 && (
                    <button onClick={clearDrawings} className="px-2 py-1 rounded text-[11px] font-semibold text-gray-600 hover:text-red-400 hover:bg-red-500/10 transition-colors flex-shrink-0">
                        Clear drawings
                    </button>
                )}
                ...
                {activeTool && (
                    <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-10 pointer-events-none select-none">
                        <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-surface-800/90 border border-edge/10 text-xs text-gray-400 backdrop-blur-sm shadow-lg">
                            ...
                            <button onClick={() => setActiveTool(null)} className="ml-2 text-gray-600 hover:text-gray-300 pointer-events-auto">ESC</button>
                        </div>
                    </div>
                )}
