...
class RiskEngine:
    ...
    async def _check_capital_exposure(
        ...
        total_capital = portfolio.available_capital + portfolio.total_invested
        new_invested = portfolio.total_invested + (price * quantity)
    ...
    async def _check_daily_loss_limit(
        ...
        if user_id in self._daily_loss_cache:
            ...
            # Calculate today's realized P&L from filled orders
            ...
            # This is a simplified check — a real system would track realized P&L per trade
            daily_loss = 0.0
